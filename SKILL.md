...
name: ecom-gpt-image
description: >-
  Create visual concepts, image-generation prompts, and AI-generated images for
  product hero images, marketing creatives, social posts, ads, ecommerce PDP visuals,
  and general visual design tasks. Supports reference image input — paste or provide a
  product photo path and get AI-generated visuals directly. Use when the user asks for
  visual strategy, image prompt writing, product/marketing image direction, or direct
  text-to-image / image-to-image generation with their own OpenAI-compatible API.
---

# ecom-gpt-image Skill

当用户需要视觉策略、图片 Prompt、商品主图、营销图、社媒图、广告图、电商 PDP 视觉，或要求直接 AI 生图时，使用这个 Skill。

支持用户直接贴图或提供图片路径，基于参考图生成高质量电商视觉。

这个 Skill 有两种模式：

1. **Brief / Prompt 模式**：只输出视觉简报和可执行图片 Prompt。
2. **Generate 模式**：当用户明确要求"生图、生成图片、出图、render image"时，先输出最终 Prompt，再调用生图脚本。
...
```bash
claude mcp add-skill https://github.com/<your-username>/ecom-gpt-image
```

或在 Claude Code 中使用 `/install https://github.com/<your-username>/ecom-gpt-image`。

### Codex

将本仓库克隆或链接到 Codex 的 skills 目录。

### 手动安装

克隆到任意位置，在 `.claude/settings.json` 或 agent 配置中引用 `SKILL.md` 路径即可。

---

## 生图配置

直接生图使用 OpenAI 兼容图像生成接口（如 GPT-Image-2，支持同步和异步轮询模式）。

### .env 配置

在以下任意位置放置 `.env` 文件（按优先级从高到低查找）：

1. 命令行 `--env-file` 参数指定的路径
2. 当前工作目录及其父目录链
3. `~/.config/ecom-gpt-image/.env`（全局配置，推荐）

```dotenv
IMG_BASE_URL=https://api.apimart.ai/v1
IMG_MODEL=gpt-image-2
IMG_API_KEY=your-api-key
IMG_OUTPUT_DIR=/Users/you/Downloads
```
...
脚本位于本 skill 安装目录下 `scripts/generate_image.py`。

```bash
# 基础用法
python3 scripts/generate_image.py --prompt "clean product hero image..."

# 从文件读取 prompt
python3 scripts/generate_image.py --prompt-file prompt.txt

# 基于参考图片生图（核心能力）
python3 scripts/generate_image.py --prompt "..." --image /path/to/product.jpg

# 多张参考图，逗号分隔
python3 scripts/generate_image.py --prompt "..." --image photo1.jpg,photo2.jpg
...
**参考图片**：用户提供产品照片时，使用 `--image` 参数传入。这是保证产品外观准确的最有效方式，比文字描述更可靠。
...
当用户在终端/对话中贴入一张图片（产品照片、素材图等）：

1. **识别图片意图**：判断用户想基于这张图做什么（商品主图、场景图、广告图、详情页等）。
2. **确认关键信息**：如果信息不足，简短确认用途、风格和平台。
3. **构建 Prompt**：以参考图为核心，写出简洁聚焦的生图 Prompt。
4. **传入 `--image`**：将用户提供的图片路径传入 `--image` 参数。
5. **生成并返回**：调用脚本生成图片，返回文件路径。
...
## 电商文案合规规则（详见 references/compliance-rules.md）

当生成的图片涉及电商商品文案（图内文字、标题、卖点、免责声明等）时，必须遵守合规规则库。按产品类型自动匹配对应规则层：
...
**关键规则摘要**：
- 禁用绝对化用语：唯一、最、第一、极致、完美、100% 等 → 替换为"多数/约/显著/之一"
- 数据宣称必须附检测报告编号
- 普通食品只能说：原料产地、工艺特点、口感风味、包装规格、品牌故事
- 所有效果表述必须添加弱化词：助力/逐步/辅助/因人而异
- 页面底部必须有对应产品类型的法律免责声明

详细禁用词替换表、免责声明模板、各平台审核重点见 `references/compliance-rules.md`。
...
6. 输出目录用产品英文 slug，例如 `<IMG_OUTPUT_DIR>/laundry-detergent-pods-pdp/`。
...
5. 如果用户提供了参考图片，传入 `--image` 参数。
6. 输出目录按优先级：`--output-dir` > `IMG_OUTPUT_DIR` 环境变量 > `generated-images/`。
7. 如果缺少 `IMG_API_KEY` 等配置，不要调用脚本；返回 Prompt 和配置命令示例。

命令形状：

```bash
python3 scripts/generate_image.py --prompt "..." --size 1:1 --resolution 2k
python3 scripts/generate_image.py --prompt "..." --image /path/to/product.jpg --size 1:1
```

脚本支持：

- `--prompt`：直接传入文字 prompt
- `--prompt-file`：从文件读取 prompt
- `--output-dir`：输出目录（覆盖 IMG_OUTPUT_DIR）
- `--size`：比例格式（`1:1`、`16:9`、`2:3`、`4:5` 等 14 种）
- `--resolution`：`1k` / `2k` / `4k`，默认 `2k`
- `--image`：参考产品图片路径（支持多张，逗号分隔）
