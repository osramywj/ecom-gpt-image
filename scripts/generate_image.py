#!/usr/bin/env python3
"""使用 OpenAI 兼容接口生成图片（自动适配同步/异步模式）。

配置来自环境变量或 `.env` 文件（按优先级查找）：
1. --env-file 参数指定的路径
2. 当前工作目录及其父目录链
3. ~/.config/ecom-gpt-image/.env（全局配置）

必需变量：
- IMG_BASE_URL: API 根地址，例如 http://127.0.0.1:8317/v1
- IMG_MODEL: 图片模型名，例如 gpt-image-2
- IMG_API_KEY: API key

可选变量：
- IMG_OUTPUT_DIR: 图片默认保存目录
"""
...
VALID_SIZES = ("auto", "1024x1024", "1536x1024", "1024x1536",
               ...
               "1792x1024", "1024x1792", "3840x2160", "2160x3840")
...
RATIO_MAP: dict[str, dict[str, str]] = {
    ...
    "16:9":  {"1k": "1792x1024", "2k": "2048x1152", "4k": "3840x2160"},
    "9:16":  {"1k": "1024x1792", "2k": "1152x2048", "4k": "2160x3840"},
    "4:3":   {"1k": "1536x1024", "2k": "1536x1024", "4k": "1536x1024"},
    "3:4":   {"1k": "1024x1536", "2k": "1024x1536", "4k": "1024x1536"},
    "4:5":   {"1k": "1024x1536", "2k": "1024x1536", "4k": "1024x1536"},
    "5:4":   {"1k": "1536x1024", "2k": "1536x1024", "4k": "1536x1024"},
    "7:4":   {"1k": "1792x1024", "2k": "2048x1152", "4k": "3840x2160"},
    "4:7":   {"1k": "1024x1792", "2k": "1152x2048", "4k": "2160x3840"},
    "21:9":  {"1k": "1792x1024", "2k": "2048x1152", "4k": "3840x2160"},
    "9:21":  {"1k": "1024x1792", "2k": "1152x2048", "4k": "2160x3840"},
...
def find_env_files() -> list[Path]:
    ...
    for directory in (Path.cwd(), *Path.cwd().parents):
        ...
        if env_file.is_file():
            ...
            break
    # 全局配置
    global_env = Path.home() / ".config" / "ecom-gpt-image" / ".env"
...
def load_all_env_files(explicit_env_file: Path | None) -> None:
    """加载所有 .env 文件。显式指定的优先，然后按查找顺序加载。"""
...
def require_config(name: str) -> str:
    ...
    fail(
        ...
        f"也兼容这些变量名：{accepted}。\n"
        f"配置文件可放在：当前目录/.env 或 ~/.config/ecom-gpt-image/.env"
    )
...
def resolve_size(size_input: str, resolution: str) -> str:
    """将比例格式或像素格式转为有效像素尺寸。"""
    # 已经是像素格式
    if "x" in size_input:
        if size_input not in VALID_SIZES:
            fail(f"--size 不合法：{size_input}。可选值：{', '.join(VALID_SIZES)}")
        return size_input
    # 比例格式
    ratio = size_input.lower().strip()
    if ratio in RATIO_MAP:
        pixel_size = RATIO_MAP[ratio].get(resolution, RATIO_MAP[ratio]["2k"])
        return pixel_size
    fail(f"--size 不合法：{size_input}。支持比例格式（如 1:1, 16:9, 2:3）或像素格式（如 1024x1024）。")
...
def build_payload(args: argparse.Namespace, prompt: str, model: str,
                  pixel_size: str) -> dict[str, Any]:
    ...
    if args.image:
        image_paths = [p.strip() for p in args.image.split(",") if p.strip()]
        payload["image_urls"] = [encode_image_as_data_uri(p) for p in image_paths]
    return payload


def http_post(url: str, api_key: str, payload: dict[str, Any],
...
def call_generate(base_url: str, api_key: str, payload: dict[str, Any],
                  timeout: int = 180) -> dict[str, Any]:
    ...
    if code and code != 200:
        ...
        fail(f"生成失败（code={code}）：{error.get('message', json.dumps(result))}")
    return result
...
def save_sync_results(result: dict[str, Any], output_dir: Path, fmt: str) -> list[Path]:
    """保存同步模式（OpenAI 标准）的响应结果。"""
    data = result.get("data")
    if not isinstance(data, list) or not data:
        fail(f"响应缺少 data 数组：{json.dumps(result)[:300]}")
    output_dir.mkdir(parents=True, exist_ok=True)
    paths: list[Path] = []
    for index, item in enumerate(data):
        if "b64_json" in item:
            img_bytes = base64.b64decode(item["b64_json"])
            out_path = output_dir / filename_for(fmt)
            out_path.write_bytes(img_bytes)
            paths.append(out_path)
        elif "url" in item:
            paths.append(download_image(item["url"], output_dir, fmt))
        else:
            fail(f"响应中第 {index} 项既无 b64_json 也无 url：{json.dumps(item)[:200]}")
    return paths


def save_async_results(task_data: dict[str, Any], output_dir: Path, fmt: str) -> list[Path]:
    """保存异步轮询模式的响应结果。"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="使用 OpenAI 兼容接口生成图片（自动适配同步/异步模式）。"
    )
    ...
    parser.add_argument(
        "--env-file",
        help="指定 .env 配置文件；不指定时自动查找。",
    )
    parser.add_argument(
        "--size",
        default="1:1",
        help="图片尺寸，支持比例格式（1:1, 16:9, 2:3, 4:5 等）或像素格式（1024x1024 等）。默认 1:1。",
    )
    parser.add_argument(
        "--resolution",
        default="2k",
        choices=("1k", "2k", "4k"),
        help="分辨率等级，仅在比例格式时生效。默认 2k。",
    )
    parser.add_argument(
        "--quality",
        default="auto",
        choices=VALID_QUALITIES,
        help="生成质量，默认 auto。可选：auto, low, medium, high。",
    )
    parser.add_argument(
        "--image",
    ...
    parser.add_argument(
        ...
        type=int,
        default=180,
        help="超时秒数（同步模式为请求超时，异步模式为轮询超时），默认 180。",
    )
    parser.add_argument(
        "--format",
...
def is_async_response(result: dict[str, Any]) -> bool:
    """判断响应是否为异步模式（包含 task_id）。"""
    data = result.get("data")
    if isinstance(data, list) and data:
        return bool(data[0].get("task_id"))
    return False
...
def main() -> None:
    ...
    start = time.time()
    result = call_generate(base_url, api_key, payload, timeout=args.timeout)

    if is_async_response(result):
        task_id = result["data"][0]["task_id"]
        print(f"异步任务已提交: {task_id}，开始轮询...", file=sys.stderr)
        time.sleep(15)
        task_data = poll_task(base_url, api_key, task_id,
        ...
        paths = save_async_results(task_data, output_dir, args.format)
    else:
        elapsed = time.time() - start
        print(f"生成完成，耗时 {elapsed:.0f}s", file=sys.stderr)
        paths = save_sync_results(result, output_dir, args.format)
